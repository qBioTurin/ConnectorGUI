...
docker.estimation.h<-function(input.file,output.folder,p)
{
  #running time 1
  ptm <- proc.time()
  ...
  params <- paste("--cidfile", paste0(data.folder, "/dockerID"),
                  ...
                  "qbioturin/connector Rscript /Scripts/Estimation_h.R",p)
  ...
  system(paste("docker logs ", substr(container.id,1,12), " &> ",data.folder,"/", "PCA_", substr(container.id,1,12),".log", sep=""))
...
docker.estimation.h(input.file=input.file,output.folder=output.folder,p=p)