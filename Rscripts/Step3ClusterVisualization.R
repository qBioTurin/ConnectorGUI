...
docker.ClusterVisual<-function(input.file,output.folder,mood,feature,title=NULL,labels.x=NULL,labels.y=NULL)
{
  #running time 1
  ptm <- proc.time()
  ...
  params <- paste("--cidfile", paste0(data.folder, "/dockerID"),
                  ...
                  "qbioturin/connector Rscript /Scripts/ClusterVisualization.R",feature,mood,labels.x,labels.y,title)
...
docker.ClusterVisual(input.file=input.file,output.folder=output.folder,mood = mood,feature=feature,title=title,labels.x=labels.x,labels.y=labels.y)
