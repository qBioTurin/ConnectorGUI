...
docker.Counting<-function(input.file,output.folder,feature)
{
  #running time 1
  ptm <- proc.time()
  ...
  params <- paste("--cidfile", paste0(data.folder, "/dockerID"),
                  ...
                  "qbioturin/connector Rscript /Scripts/Counting.R",feature)
...
docker.Counting(input.file=input.file,output.folder=output.folder,feature=feature)
