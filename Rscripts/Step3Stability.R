...
docker.ClusterStability<-function(input.file,output.folder,p,h,kmin,kmax,runs,seed=2404)
{
  #running time 1
  ptm <- proc.time()
  ...
  params <- paste("--cidfile", paste0(data.folder, "/dockerID"),
                  ...
                  "qbioturin/connector Rscript /Scripts/ClusteringStability.R",p,h,kmin,kmax,runs,seed)
...
docker.ClusterStability(input.file=input.file,output.folder=output.folder,p=p,h=h,kmin=kmin,kmax=kmax,runs=runs,seed=2404)
